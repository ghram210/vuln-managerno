...
add_arabic_paragraph(doc, '2.4.2 أنظمة التشغيل المستخدمة في المشروع (Windows وLinux)', is_heading=True, level=2)
add_arabic_paragraph(doc, "في هذا المشروع تم الاعتماد على نظامي تشغيل مختلفين:")
add_arabic_paragraph(doc, "• Windows: يُستخدم كنظام تشغيل أساسي لتطوير واجهة المستخدم وإدارة المشروع، حيث يوفر بيئة مريحة لتطوير الويب باستخدام VS Code.")
add_arabic_paragraph(doc, "• Linux (Kali Linux): يُستخدم كنظام متخصص في الأمن السيبراني لتشغيل أدوات مثل Nmap وSQLmap وFFUF بسبب قوته في التعامل مع الشبكات واستقراره [10].")

add_arabic_paragraph(doc, '2.5.1 بيئات العمل (Development & Execution Environments)', is_heading=True, level=1)
add_arabic_paragraph(doc, "1. بيئة التطوير (Windows): حيث يتم كتابة كود الواجهة الأمامية، كود الخادم (FastAPI)، وإدارة الكود.")
add_arabic_paragraph(doc, "2. بيئة التنفيذ والفحص (Kali Linux): حيث يتم تنفيذ أدوات فحص الثغرات، إجراء المسح الفعلي، وجمع النتائج الخام.")
