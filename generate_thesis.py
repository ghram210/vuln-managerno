...
add_arabic_paragraph(doc, '2.5.1 أتمتة الفحص والتوافقية التشغيلية في بيئة Kali Linux', is_heading=True, level=1)
add_arabic_paragraph(doc, "تم استغلال نظام Kali Linux كمنصة تشغيلية قوية لإدارة وأتمتة دورة حياة فحص الثغرات. يعتمد المشروع على القدرات المتقدمة للنظام في التعامل مع سطر الأوامر (CLI) لتشغيل أدوات الفحص المتعددة بشكل متزامن. يتم استخدام الخادم الخلفي (FastAPI) كمحرك أتمتة يقوم بمعالجة أوامر التنفيذ بذكاء؛ حيث يقوم النظام بفحص مستوى صلاحيات المستخدم الحالي وتعديل معايير تشغيل الأدوات تلقائياً لضمان التوافقية (على سبيل المثال، التحويل التلقائي بين أنواع فحص Nmap بناءً على توفر صلاحيات الـ Root). تتيح هذه البيئة للنظام إدارة العمليات الخلفية (Processes) بكفاءة، والتقاط تدفق البيانات الخام في الوقت الفعلي، وضمان استقرار التواصل بين طبقات التطبيق البرمجية والأدوات الأمنية التي تتطلب وصولاً منخفض المستوى (Low-level access) إلى بروتوكولات الشبكة، مما يضمن دقة عالية في استخراج النتائج وتحليلها [10].")

add_arabic_paragraph(doc, '2.5.2 التكامل الهيكلي: واجهات البرمجة API والتفاعل التنفيذي ABI', is_heading=True, level=1)
