...
add_arabic_paragraph(doc, '2.5.1 أتمتة الفحص وإدارة الموارد في بيئة Kali Linux', is_heading=True, level=1)
add_arabic_paragraph(doc, "تم استغلال نظام Kali Linux كمنصة تشغيلية قوية لإدارة وأتمتة دورة حياة فحص الثغرات. يعتمد المشروع على القدرات المتقدمة للنظام في التعامل مع سطر الأوامر (CLI) لتشغيل أدوات الفحص المتعددة بشكل متزامن. يتم استخدام الخادم الخلفي (FastAPI) كـ \"أوركسترا\" تقنية تقوم بإرسال أوامر التنفيذ المباشرة إلى نواة النظام، مما يسمح بتشغيل أدوات مثل Nmap و SQLmap بصلاحيات محددة، والتقاط تدفق البيانات الخام (Raw Data Streams) في الوقت الفعلي. تتيح هذه البيئة للنظام إدارة الموارد بكفاءة، حيث يتم التحكم في العمليات الخلفية (Processes)، ومراقبة استهلاك الذاكرة، وضمان استقرار التواصل بين طبقات التطبيق البرمجية والأدوات الأمنية التي تتطلب وصولاً منخفض المستوى (Low-level access) إلى بروتوكولات الشبكة، مما يضمن دقة عالية في استخراج النتائج وتحليلها [10].")
