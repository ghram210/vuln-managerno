...
add_arabic_paragraph(doc, '2.5.1 بيئة المختبر الأمني الموحدة (Unified Security Lab Environment)', is_heading=True, level=1)
add_arabic_paragraph(doc, "تعتمد استراتيجية تنفيذ هذا المشروع على مفهوم \"المختبر الأمني الموحد\" باستخدام نظام Kali Linux كبيئة تطوير وتنفيذ شاملة. بدلاً من توزيع المكونات على أنظمة تشغيل مختلفة، تم دمج أدوات التطوير (مثل VS Code ومترجمات Python) مع أدوات الفحص الأمنية في بيئة واحدة. هذا النهج يقلل بشكل كبير من احتمالية حدوث أخطاء ناتجة عن عدم توافق المكتبات البرمجية أو اختلاف إصدارات بروتوكولات الشبكة بين بيئة البرمجة وبيئة الفحص الفعلي. كما تتيح هذه البيئة الموحدة تحكماً كاملاً في موارد النظام، مما يسهل عملية أتمتة الأدوات (Automation) وضمان استقرار التواصل بين الخادم (FastAPI) والمكونات البرمجية للأدوات الأمنية الثقيلة [10].")
