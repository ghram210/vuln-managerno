...
intro_text = (
    ...
    "مع تحول حياتنا إلى الفضاء الإلكتروني، لم يعد هذا الترف رفاهية، بل أصبح درعًا واقيًا يحفظ خصوصيتنا ويحمي أصولنا الرقمية من الاستغلال [2]."
)
...
imp_text = (
    ...
    "حيث يمثل الاكتشاف المبكر للثغرات خط الدفاع الأول لحماية البيانات من التسرب غير المصرح به، والحفاظ على سلامة (Integrity) البيانات من العبث أو التعديل، "
    "وضمان استمرارية عمل الخدمات الإلكترونية دون انقطاع [3]. يؤدي إهمال هذا الجانب الحاسم إلى خروقات أمنية شاملة، "
    "تتراوح بين سرقة البيانات الحساسة، تشويه المحتوى، أو تعطيل المنصات الرقمية بالكامل، مما يهدد مصداقية المنظمات واستقرار عملياتها الرقمية [1]."
)
...
add_arabic_paragraph(doc, 'شروحات واجهة الأصول الرقمية (Assets Dashboard)', is_heading=True, level=1)
img_exps = [
    "1. الجزء الأول (نظرة عامة على المخاطر): تعرض الرسوم البيانية توزيع الثغرات المكتشفة بناءً على مستويات الخطورة (Critical, High, Medium) ونوع الأداة المستخدمة في الكشف.",
    "2. الجزء الثاني (تحليل الاستغلالية وناقل الهجوم): يوضح نسبة الثغرات التي تمتلك أكواد استغلال معلنة (Exploitability) والمسار الذي يسلكه المهاجم للوصول إلى الثغرة (Attack Vector).",
    "3. الجزء الثالث (سجل الأصول الممسوحة): جدول تفصيلي يعرض عناوين IP للمواقع المستهدفة، الأدوات المستخدمة، المنافذ المفتوحة، وتقييم الخطورة لكل أصل رقمي."
]
for exp in img_exps:
    add_arabic_paragraph(doc, exp)
...
doc.save('الفصل_الأول_مع_المراجع.docx')
