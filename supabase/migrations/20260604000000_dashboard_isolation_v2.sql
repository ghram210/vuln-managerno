-- Fix dashboard views isolation and track daily discovery trend
BEGIN;

-- 1. Helper function for multi-tenant isolation
CREATE OR REPLACE FUNCTION public.get_inviter_id(uuid) RETURNS uuid AS $$
  SELECT il.created_by
  FROM public.invitation_links il
  JOIN public.invitation_usages iu ON iu.invitation_id = il.id
  WHERE iu.user_id = $1
  LIMIT 1;
$$ LANGUAGE sql STABLE SECURITY DEFINER;

-- 2. Update RLS policies to allow shared access for invited users
DROP POLICY IF EXISTS "scan_results_isolation" ON public.scan_results;
CREATE POLICY "scan_results_isolation" ON public.scan_results FOR ALL TO authenticated
USING (user_id = auth.uid() OR user_id = public.get_inviter_id(auth.uid()));

DROP POLICY IF EXISTS "scan_findings_isolation" ON public.scan_findings;
CREATE POLICY "scan_findings_isolation" ON public.scan_findings FOR ALL TO authenticated
USING (EXISTS (
    SELECT 1 FROM public.scan_results sr
    WHERE sr.id = scan_id
    AND (sr.user_id = auth.uid() OR sr.user_id = public.get_inviter_id(auth.uid()))
));

DROP POLICY IF EXISTS "finding_cves_isolation" ON public.finding_cves;
CREATE POLICY "finding_cves_isolation" ON public.finding_cves FOR ALL TO authenticated
USING (EXISTS (
...
-- 3. Redefine views to REMOVE hardcoded user filters and use DISCOVERY trend logic
...
-- Re-create views without explicit user_id filters, relying on RLS
...
targets AS ( SELECT DISTINCT f.target FROM public.scan_findings f )
SELECT md5(d.day_date::text || COALESCE(t.target, 'all'))::uuid AS id, t.target, d.day_num AS day, COUNT(DISTINCT f.id)::int AS count
FROM days d CROSS JOIN targets t LEFT JOIN public.scan_findings f ON f.created_at::date = d.day_date AND f.target = t.target
...
FROM public.scan_findings f GROUP BY f.target, f.status;
